...
角色定义从 prompts/agents/architect.md 动态加载。
任务提示词从 prompts/design.md 动态加载。
"""
...
class WriteDesign(Action):
    ...
    async def run(self, prd: str, scope: str = "mvp") -> str:
        ...
        """
        role = load_prompt("agents/architect")
        task = load_prompt("design")
        prompt = f"{role}\n\n{task}".format(prd=prd, scope=scope)
